...
const RealmGenerator = ({ rows = 12, cols = 12 }) => {
  ...
  const handleHexClick = (row, col) => {
    ...
    const currentIndex = terrainTypes.findIndex(t => t.type === currentHex.terrainType.type);
    const nextIndex = (currentIndex + 1) % terrainTypes.length;
    
    realm.setHex(row, col, terrainTypes[nextIndex]);
    setRealm(new Realm(realm.rows, realm.cols));
    const newRealm = new Realm(realm.rows, realm.cols);
    newRealm.hexMap = realm.hexMap;
    newRealm.holdings = realm.holdings;
    newRealm.landmarks = realm.landmarks;
    newRealm.myths = realm.myths;
    setRealm(newRealm);
  };
